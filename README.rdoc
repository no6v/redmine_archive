...
This plugin depends on Setting#mail_handler_api_key to encrypt/decrypt
the mail key, so ensure to generate mail_handler_api_key even if you
do not enable the web service for incoming emails.

= Updating the articles table

Use bin/importer.rb to import new mails. This script scans its
